  ...
  try {
    response = await client.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `Research this person, studio, or organization for a cultural-landscape database entry: "${query.trim()}". Use web search to find their role/category, location (city + country), a factual 2-4 sentence bio, career highlights, up to 3 real notable works or projects (with URLs when you find them), and social/contact links (main website, secondary website if different, Instagram, Twitter/X, TikTok). Be efficient: use as few searches as possible - prefer 1-2 broad searches (e.g. their name plus site/profile) that cover most of these fields at once over one search per field, and stop searching as soon as you have enough to proceed. If the input is a URL on a platform that often blocks search indexing/crawling (e.g. Instagram, Twitter/X, Facebook, TikTok), still try it first - profile bios are one of the best sources for a mainUrl/website link - but if it's inaccessible or returns nothing useful after one attempt, don't keep retrying it; fall back to searching by the handle or name plus platform instead (e.g. "\"name\" instagram") to find whatever is publicly indexed about them elsewhere. Once you've gathered what you can, call record_entrant exactly once with your findings. If some fields can't be found, still call record_entrant and leave those fields null/empty rather than guessing or searching further.`,
        },
      ],
    });
