  ...
  try {
    response = await client.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `Research this person, studio, or organization for a cultural-landscape database entry: "${query.trim()}". Find their role/category, location (city + country), a factual 2-4 sentence bio, career highlights, up to 3 real notable works or projects (with URLs when you find them), and social/contact links (main website, secondary website if different, Instagram, Twitter/X, TikTok). If the input is a specific URL, fetch it directly first (web_fetch) to read its actual content - that's faster and more reliable than searching around it, and is exactly what it's for. Use web_search for anything the fetched page doesn't cover, or when starting from just a name with no URL. Be efficient: as few tool calls as possible - prefer one fetch and/or 1-2 broad searches that cover most fields at once over one call per field, and stop as soon as you have enough to proceed. If the input is a URL on a platform that often blocks fetching/indexing (e.g. Instagram, Twitter/X, Facebook, TikTok), still try fetching it first - profile bios are one of the best sources for a mainUrl/website link - but if it's inaccessible or returns nothing useful after one attempt, don't keep retrying it; fall back to searching by the handle or name plus platform instead (e.g. "\"name\" instagram") to find whatever is publicly indexed about them elsewhere. Once you've gathered what you can, call record_entrant exactly once with your findings. If some fields can't be found, still call record_entrant and leave those fields null/empty rather than guessing or searching further.`,
        },
      ],
    });
